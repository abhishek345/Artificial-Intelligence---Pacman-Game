...
def enhancedFeatureExtractorDigit(datum):
    ...
    feature = 0
    #features = util.Counter()
    widthRange = range(DIGIT_DATUM_WIDTH)
    heightRange = range(DIGIT_DATUM_HEIGHT)
    for x in widthRange:
        continousBlocks = 0
        pixelDensity = datum.getPixel(x,0)
        for y in heightRange:
            newPixelDensity = datum.getPixel(x, y)
            if newPixelDensity != pixelDensity:
                continousBlocks += 1
                pixelDensity = newPixelDensity
        for i in range(8):
            if continousBlocks == i:
                features[feature] = 1
            else:
                features[feature] = 0
        feature += 1
