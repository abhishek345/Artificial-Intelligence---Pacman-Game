...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        newDists = util.Counter()
        for p in self.beliefs.keys():
            newDists[p] = newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
        for oldPos in self.beliefs.keys():
            for newPos, prob in newDists[oldPos].items():
    ...
    def observe(self, observation, gameState):
        ...
        #update
            ...
            for pos in self.legalPositions:
                ...
                allPossible[pos] += emissionModel[dist]*belief[pos]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for p in range(self.numParticles):
            self.particles.append(permutations[p % len(permutations)])
    ...
    def observeState(self, gameState):
        ...
        for p in range(len(self.particles)):
            for g in range(len(self.particles[p])):
                if noisyDistances[g] is None:
                    self.particles[p] = self.getParticleWithGhostInJail(self.particles[p], g) # account for ghosts who have been eaten

        weights = util.Counter()
        particleValues = set(self.particles)
        for v in particleValues:
            weights[v] = 1 # multiplicative identity
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    pass # equivalent to multiplying by 1 for each ghost who has been eaten
                else:
                    trueDistance = util.manhattanDistance(v[i], pacmanPosition)
                    weights[v] *= emissionModels[i][trueDistance] # calculate weight to be product of weights
            weights[v] *= self.particles.count(v) # aggregate

        if weights.totalCount() == 0:
            self.initializeParticles() # account for all 9 weights
            return
 
        weights.normalize()
        newParticles = []
        for i in range(self.numParticles):
            newParticles.append(util.sample(weights)) # sample
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        belief = util.Counter() # same as in normal particle filter
        particleValues = set(self.particles)
        for v in particleValues:
            belief[v] = self.particles.count(v) / float(self.numParticles)
        return belief
